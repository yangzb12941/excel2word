...
public class TableElementHandler implements IElementHandler<TableRenderData>{
    ...
    private String[][] object2Array(CellContext cellContext){
        List itmes = cellContext.getItmes();
        String[][] table = new String[itmes.size()][9];
        for (int i = 0; i < itmes.size(); i++) {
            if(i==0){
                ...
                table[i][0] = ((WordTableModelEntity)itmes.get(i)).get账户();
                table[i][1] = ((WordTableModelEntity)itmes.get(i)).get营业部();
                table[i][2] = ((WordTableModelEntity)itmes.get(i)).get客户类型();
                table[i][3] = ((WordTableModelEntity)itmes.get(i)).get使用系统();
                table[i][4] = ((WordTableModelEntity)itmes.get(i)).get服务人员姓名();
                table[i][5] = ((WordTableModelEntity)itmes.get(i)).get服务人员编号();
                table[i][6] = ((WordTableModelEntity)itmes.get(i)).get服务人员团队();
                table[i][7] = ((WordTableModelEntity)itmes.get(i)).get月份();
                table[i][8] = ((WordTableModelEntity)itmes.get(i)).get批次();
            }
        }
        return table;
    }
